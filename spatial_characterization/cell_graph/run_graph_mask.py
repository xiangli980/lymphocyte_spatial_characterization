...
def read_files(img_path, msk_path, mat_path):
    """Read image and mat files"""
    img = skio.imread(img_path, plugin='pil')
    msk = skio.imread(msk_path, plugin='pil')
    mask = msk<255     # convert mask to 0-255 range 
    mask = img_as_ubyte(mask)[:,:,0] # binary mask 
    inst_info = sio.loadmat(mat_path)
    inst_centroids = inst_info['inst_centroid']
    inst_types = inst_info['inst_type']
    return img, mask, inst_centroids, inst_types
...
def draw_graph(g, img, points, name, output_dir, marker_size, line_width, cell_dist=40):
    ...
    gt.graph_draw(g, pos=g.vp['pos'], vprops = {"fill_color" : 'yellow', "size" :marker_size}, 
                  ...
                  output = os.path.join(output_dir, 'graph', '{}_graph_{}.svg'.format(name, cell_dist)))
    
def process_graph(inst_centroids, inst_types, msk, name, cell_dist=40):
    ...
    for centroid, inst_type in zip(inst_centroids, inst_types):
        ...
        if inst_type[0] == 1: # cell type is non-lymphocyte
            x0, y0 = centroid[0], centroid[1]
            # if y0>=22000 or x0>=20000:
            #     continue
            # check if the point is within the msk (10x downsized)
            if msk[int(y0/10), int(x0/10)] == 255:
                points.append([x0,y0])
...
def main():
    ...
    parser.add_argument('--input_dir_msk', type=str, required=True, 
                        help='Directory containing ROI mask files')
    ...
    for name in tqdm(names):
        ...
        msk_path = os.path.join(args.input_dir_msk, f'{name}.png')
        cell_dist = args.cell_dist
        
        # Read files
        img, msk, inst_centroids, inst_types = read_files(img_path, msk_path, mat_path)
        ...
        g, f, points = process_graph(inst_centroids, inst_types, msk, name, cell_dist)
